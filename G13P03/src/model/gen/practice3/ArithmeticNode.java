...
public class ArithmeticNode implements Callback {
	...
	public String pretty() {
		...
		if (leftBranch == null)
			...
			String aux = leftBranch.pretty();
			if (knot == ArithmeticEnum.MUL && leftBranch.knot != ArithmeticEnum.MUL && aux.length() != 1)
				aux = addParentisis(aux);
			s += aux;
		}
		s += " " + knot.toString() + " ";
		if (rightBranch == null)
			...
			String aux = rightBranch.pretty();
			if (knot == ArithmeticEnum.MUL && rightBranch.knot != ArithmeticEnum.MUL && aux.length() != 1)
				aux = addParentisis(aux);
			s += aux;
		}
		return s;
